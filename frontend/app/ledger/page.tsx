...
import { uploadLedgerCSV, runDetection, getExceptions, updateCaseStatus } from '@/lib/api';
...
export default function LedgerAnalysisPage() {
  ...
  useEffect(() => {
    const id = localStorage.getItem('ledger_dataset_id');
    if (id) {
      setDatasetId(id);
      fetchExceptions();
    }
  ...
  const fetchExceptions = async () => {
    try {
      setLoading(true);
      const data = await getExceptions({ domain: 'ledger', limit: 100 });
      setExceptions(data);
    } catch (error) {
  ...
  const handleUpload = async (file: File) => {
    try {
      const res = await uploadLedgerCSV(file);
      if (res.dataset_id) {
        setDatasetId(res.dataset_id);
        localStorage.setItem('ledger_dataset_id', res.dataset_id);
        toast.success('Ledger dataset uploaded successfully');
  ...
  const handleRunDetection = async () => {
    ...
    try {
      ...
      await runDetection({ domain: 'ledger', dataset_id: datasetId, thresholds, run_by: user });
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 bg-white min-h-screen">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Ledger Transaction Analysis</h1>
          <p className="text-gray-500">Detect anomalies and fraud in general ledger records.</p>
        </div>
      ...
      {!datasetId && (
        <div className="bg-slate-50 border border-gray-200 rounded-xl p-12 text-center">
          ...
          <p className="text-gray-500 mb-6">Upload a ledger CSV to begin analysis. Required columns: vendor, amount.</p>
              ...
              {showThresholds && (
                <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                  <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Slider label="Benford Sensitivity" value={thresholds.benford_sensitivity} onChange={(v) => setThresholds({...thresholds, benford_sensitivity: v})} min={0} max={1} step={0.1} />
                    <Slider label="Duplicate Similarity (%)" value={thresholds.duplicate_similarity_threshold} onChange={(v) => setThresholds({...thresholds, duplicate_similarity_threshold: v})} min={0} max={100} step={1} />
                    <Slider label="3-Way Match Tolerance (%)" value={thresholds.three_way_match_tolerance_pct} onChange={(v) => setThresholds({...thresholds, three_way_match_tolerance_pct: v})} min={0} max={0.1} step={0.01} />
      ...
      {isUploadModalOpen && (
        <UploadModal 
          ...
          description="Required columns: vendor, amount. Optional: invoice_number, invoice_date, po_reference, po_amount, po_quantity, gr_reference, gr_quantity"
        />
      )}
    </div>
  );
